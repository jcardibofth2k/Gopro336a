...
extends Module {
    ...
    public void onEnable() {
        ...
        this.Field1708 = null;
        this.Field1709 = null;
        this.Field1706 = Field1695.getValue();
    }
    ...
    public void Method135(UpdateEvent updateEvent) {
        ...
        if (updateEvent.isCanceled() || !Class496.Method1959(Field1700.getValue())) {
            return;
        }
        if (!(!Field1699.getValue().booleanValue() || Class288.mc.player.onGround && Class288.mc.player.collidedVertically)) {
            return;
        }
        if (Class167.Method1610(PacketFly.class).isEnabled()) {
            return;
        }
        if (this.Field1706 < Field1695.getValue()) {
            ++this.Field1706;
        }
        ...
        if (blockPos != null) {
            this.Field1708 = Class496.Method1962(blockPos, Field1700.getValue());
            if (this.Field1708 != null) {
        ...
        } else if (Field1701.getValue().booleanValue()) {
            this.toggle();
            return;
        }
    }
    ...
    public static Float Method1622(EntityPlayer entityPlayer) {
        return Float.valueOf(Class288.mc.player.getDistance(entityPlayer));
    }
    ...
    public EntityPlayer Method1623() {
        ...
        if (Field1702.getValue().booleanValue()) {
            stream = Stream.concat(Class288.mc.world.playerEntities.stream(), waypoints.Method1799().keySet().stream());
        }
        return stream.filter(Class288::Method128).filter(Class288::Method122).filter(Class288::Method141).filter(Class288::Method126).filter(this::Method132).min(Comparator.comparing(Class288::Method1622)).orElse(MathUtil.Method1087(Field1698.getValue().Method851()) ? Class288.mc.player : null);
    }
    ...
    public static boolean Method126(EntityPlayer entityPlayer) {
        return Class288.mc.player.getDistance(entityPlayer) < Math.max(Field1693.getValue().floatValue() - 1.0f, 1.0f);
    }
    ...
    public void Method123(Class50 class50) {
        block9: {
            ...
            if (bl) {
                ...
                Class288.mc.player.connection.sendPacket(new CPacketHeldItemChange(this.Field1703));
            }
            boolean bl2 = Class288.mc.player.isSprinting();
            boolean bl3 = Class545.Method1004(this.Field1708.Method1982());
            if (bl2) {
                Class288.mc.player.connection.sendPacket(new CPacketEntityAction(Class288.mc.player, CPacketEntityAction.Action.STOP_SPRINTING));
            }
            if (bl3) {
                Class288.mc.player.connection.sendPacket(new CPacketEntityAction(Class288.mc.player, CPacketEntityAction.Action.START_SNEAKING));
            }
            Class496.Method1957(this.Field1708, EnumHand.MAIN_HAND, true);
            for (int i = 0; i < Field1694.getValue() - 1; ++i) {
                BlockPos blockPos = this.Method1626(this.Field1707);
                if (blockPos != null) {
                    Class490 class490 = Class496.Method1961(blockPos, Field1700.getValue(), true);
                ...
                if (!Field1701.getValue().booleanValue()) break;
                this.toggle();
                if (bl) {
                    Class288.mc.player.inventory.currentItem = n;
                    Class288.mc.player.connection.sendPacket(new CPacketHeldItemChange(n));
                }
                return;
            }
            if (bl3) {
                Class288.mc.player.connection.sendPacket(new CPacketEntityAction(Class288.mc.player, CPacketEntityAction.Action.STOP_SNEAKING));
            }
            if (bl2) {
                Class288.mc.player.connection.sendPacket(new CPacketEntityAction(Class288.mc.player, CPacketEntityAction.Action.START_SPRINTING));
            }
            if (bl) {
                Class288.mc.player.inventory.currentItem = n;
                Class288.mc.player.connection.sendPacket(new CPacketHeldItemChange(n));
    ...
    public BlockPos Method1626(BlockPos blockPos) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            blockPos3 = null;
            d2 = 0.0;
            if (this.Method1624(blockPos.up().offset(enumFacing), false) && (!Field1697.getValue().booleanValue() || this.Method1572(blockPos.up(), enumFacing)) && (d = Class288.mc.player.getDistance((double)(blockPos2 = blockPos.up().offset(enumFacing)).getX() + 0.5, (double)blockPos2.getY() + 0.5, (double)blockPos2.getZ() + 0.5)) >= d2) {
                blockPos3 = blockPos2;
                d2 = d;
            }
            if (blockPos3 == null) continue;
        ...
        if (Field1696.getValue().booleanValue() && ((block = Class288.mc.world.getBlockState(blockPos.up().up()).getBlock()) instanceof BlockAir || block instanceof BlockLiquid)) {
            if (this.Method1624(blockPos.up().up(), false)) {
                return blockPos.up().up();
            }
            BlockPos blockPos4 = blockPos.up().up().offset(EnumFacing.byHorizontalIndex((int)(MathHelper.floor((double)(Class288.mc.player.rotationYaw * 4.0f / 360.0f) + 0.5) & 3)));
