...
public class Entity1_13Types {
   public static Entity1_13Types.EntityType getTypeFromId(int typeID, boolean isObject) {
      Optional type;
      if (isObject) {
         type = Entity1_13Types.ObjectType.getPCEntity(typeID);
      } else {
         type = Entity1_13Types.EntityType.findById(typeID);
      }

      if (!type.isPresent()) {
         Via.getPlatform().getLogger().severe("Could not find 1.13 type id " + typeID + " isObject=" + isObject);
         return Entity1_13Types.EntityType.ENTITY;
      } else {
         return (Entity1_13Types.EntityType)type.get();
      }
   }
   ...
   enum ObjectType implements com.viaversion.viaversion.api.minecraft.entities.ObjectType {
      ...
      public static Optional findById(int id) {
         return id == -1 ? Optional.empty() : Optional.ofNullable((Entity1_13Types.ObjectType)TYPES.get(id));
      }

      public static Optional getPCEntity(int id) {
         Optional output = findById(id);
         return !output.isPresent() ? Optional.empty() : Optional.of(((Entity1_13Types.ObjectType)output.get()).type);
      }

      public static Optional fromEntityType(Entity1_13Types.EntityType type) {
         Entity1_13Types.ObjectType[] var1 = values();
         int var2 = var1.length;

         for(int var3 = 0; var3 < var2; ++var3) {
            Entity1_13Types.ObjectType ent = var1[var3];
            if (ent.type == type) {
      ...
      static {
         Entity1_13Types.ObjectType[] var0 = values();
         int var1 = var0.length;

         for(int var2 = 0; var2 < var1; ++var2) {
            Entity1_13Types.ObjectType type = var0[var2];
            TYPES.put(type.field_59, type);
         }
   ...
   enum EntityType implements com.viaversion.viaversion.api.minecraft.entities.EntityType {
      ...
      public Entity1_13Types.EntityType getParent() {
         return this.parent;
      }

      public static Optional findById(int id) {
         return id == -1 ? Optional.empty() : Optional.ofNullable((Entity1_13Types.EntityType)TYPES.get(id));
      }

      static {
         Entity1_13Types.EntityType[] var0 = values();
         int var1 = var0.length;

         for(int var2 = 0; var2 < var1; ++var2) {
            Entity1_13Types.EntityType type = var0[var2];
            TYPES.put(type.field_114, type);
         }
