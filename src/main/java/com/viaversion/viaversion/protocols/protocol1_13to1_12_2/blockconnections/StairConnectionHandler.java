...
public class StairConnectionHandler extends ConnectionHandler {
   ...
   static ConnectionData.ConnectorInitAction init() {
      List baseStairs = new LinkedList();
   ...
   public int connect(UserConnection user, Position position, int blockState) {
      StairConnectionHandler.StairData stairData = (StairConnectionHandler.StairData)stairDataMap.get(blockState);
      if (stairData == null) {
         return blockState;
      } else {
         short s = 0;
         ...
         s = (short)(s | stairData.getType() << 4);
         s = (short)(s | stairData.getFacing().ordinal() << 9);
         Integer newBlockState = (Integer)connectedBlocks.get(s);
   ...
   private int getShape(UserConnection user, Position position, StairConnectionHandler.StairData stair) {
      ...
      StairConnectionHandler.StairData relativeStair = (StairConnectionHandler.StairData)stairDataMap.get(this.getBlockData(user, position.getRelative(facing)));
      BlockFace facing2;
      ...
      relativeStair = (StairConnectionHandler.StairData)stairDataMap.get(this.getBlockData(user, position.getRelative(facing.opposite())));
      if (relativeStair != null && relativeStair.isBottom() == stair.isBottom()) {
         facing2 = relativeStair.getFacing();
   ...
   private boolean checkOpposite(UserConnection user, StairConnectionHandler.StairData stair, Position position, BlockFace face) {
      StairConnectionHandler.StairData relativeStair = (StairConnectionHandler.StairData)stairDataMap.get(this.getBlockData(user, position.getRelative(face)));
