...
public class DoorConnectionHandler extends ConnectionHandler {
   ...
   public int connect(UserConnection user, Position position, int blockState) {
      DoorConnectionHandler.DoorData doorData = (DoorConnectionHandler.DoorData)doorDataMap.get(blockState);
      if (doorData == null) {
         return blockState;
      } else {
         short s = 0;
         ...
         if (doorData.isLower()) {
            lowerHalf = (DoorConnectionHandler.DoorData)doorDataMap.get(this.getBlockData(user, position.getRelative(BlockFace.TOP)));
            if (lowerHalf == null) {
               return blockState;
            }
            ...
            lowerHalf = (DoorConnectionHandler.DoorData)doorDataMap.get(this.getBlockData(user, position.getRelative(BlockFace.BOTTOM)));
            if (lowerHalf == null) {
               return blockState;
            }
         ...
         Integer newBlockState = (Integer)connectedStates.get(s);
   ...
   private static final class DoorData {
      ...
      // $FF: synthetic method
      DoorData(boolean x0, boolean x1, boolean x2, boolean x3, BlockFace x4, int x5, 1 x6) {
