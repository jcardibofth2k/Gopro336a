      ...
      Via.getPlatform().runAsync(() -> {
         ...
         if (message != null) {
            Via.getPlatform().runSync(() -> {
               Via.getPlatform().sendMessage(uuid, "§a§l[ViaVersion] §a" + message);
            });
         }
      ...
      if (Via.getPlatform().getPluginVersion().equals("${version}")) {
         ...
         if (newestString == null) {
            ...
            if (current.compareTo(newest) < 0) {
               ...
               return !tag.startsWith("dev") && !tag.startsWith("snapshot") ? "You are running a newer version of the plugin than is released!" : "You are running a development version of the plugin, please report any bugs to GitHub.";
            } else {
               return null;
            }
         }
