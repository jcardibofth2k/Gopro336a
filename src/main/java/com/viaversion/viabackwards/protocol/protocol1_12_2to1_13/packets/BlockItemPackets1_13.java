...
public class BlockItemPackets1_13 extends ItemRewriter {
               ...
               while(var8.hasNext()) {
                  ...
                  if (idTag != null) {
                     ...
                     if (provider.isHandled(id)) {
                        x = ((NumberTag)tag.get("y")).asInt() >> 4;
                        if (x >= 0 && x <= 15) {
                           ChunkSection section = chunk.getSections()[x];
                           int xx = ((NumberTag)tag.get("x")).asInt();
                           int yx = ((NumberTag)tag.get("y")).asInt();
                           int zx = ((NumberTag)tag.get("z")).asInt();
                           Position position = new Position(xx, (short)yx, zx);
                           int blockx = section.getFlatBlock(xx & 15, yx & 15, zx & 15);
               ...
               for(i = 0; i < chunk.getSections().length; ++i) {
                  ...
                  if (sectionx != null) {
                     ...
                     for(y = 0; y < 16; ++y) {
                        for(z = 0; z < 16; ++z) {
                           for(x = 0; x < 16; ++x) {
                              int block = sectionx.getFlatBlock(x, y, z);
                     ...
                     for(y = 0; y < sectionx.getPaletteSize(); ++y) {
                        ...
                        if (z != 0) {
                           x = ((Protocol1_12_2To1_13)BlockItemPackets1_13.this.protocol).getMappingData().getNewBlockStateId(z);
                           sectionx.setPaletteEntry(y, x);
                        }
                     }
                  }
               }
   ...
   private void rewriteEnchantmentsToClient(CompoundTag tag, boolean storedEnch) {
      ...
      if (enchantments != null) {
         ...
         Iterator var9 = enchantments.clone().iterator();
                     ...
                     if (noMapped.size() != 0) {
                        ...
                        if (!lore.isEmpty()) {
                           ...
                           if (loreTag == null) {
                              ...
                              Iterator var26 = loreTag.iterator();

                              while(var26.hasNext()) {
                                 Tag value = (Tag)var26.next();
         ...
         if (tag != null) {
            ...
            if (display instanceof CompoundTag) {
               ...
               if (identifier != null) {
                  ...
                  identifier.setValue(via != null ? via.getValue() : ChatRewriter.legacyTextToJsonString(identifier.getValue()));
               }
            }
