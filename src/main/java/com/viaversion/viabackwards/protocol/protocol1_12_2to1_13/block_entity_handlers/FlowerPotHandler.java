...
public class FlowerPotHandler implements BackwardsBlockEntityProvider.BackwardsBlockEntityHandler {
   ...
   public Pair getOrDefault(int blockId) {
      Pair pair = (Pair)FLOWERS.get(blockId);
      return pair != null ? pair : AIR;
   }

   public CompoundTag transform(UserConnection user, int blockId, CompoundTag tag) {
      Pair item = this.getOrDefault(blockId);
      tag.put("Item", new StringTag((String)item.getKey()));
      tag.put("Data", new IntTag((Byte)item.getValue()));
      return tag;
   }
