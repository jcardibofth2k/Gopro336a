...
public abstract class LegacySoundRewriter extends RewriterBase {
   ...
   public int handleSounds(int soundId) {
      ...
      LegacySoundRewriter.SoundData data = (LegacySoundRewriter.SoundData)this.soundRewrites.get(soundId);
      if (data != null) {
         return data.getReplacementSound();
      } else {
         ObjectIterator var4 = this.soundRewrites.int2ObjectEntrySet().iterator();

         while(var4.hasNext()) {
            Int2ObjectMap.Entry entry = (Int2ObjectMap.Entry)var4.next();
            if (soundId > entry.getIntKey()) {
               if (((LegacySoundRewriter.SoundData)entry.getValue()).isAdded()) {
                  --newSoundId;
               } else {
                  ++newSoundId;
               }
