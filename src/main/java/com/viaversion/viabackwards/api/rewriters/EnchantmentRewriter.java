...
public class EnchantmentRewriter {
   ...
   public void rewriteEnchantmentsToClient(CompoundTag tag, boolean storedEnchant) {
      ...
      List loreToAdd = new ArrayList();
      boolean changed = false;
      Iterator iterator = enchantments.iterator();

      CompoundTag display;
      while(iterator.hasNext()) {
         display = (CompoundTag)iterator.next();
         StringTag idTag = (StringTag)display.get("id");
         if (idTag != null) {
            ...
            if (remappedName != null) {
               ...
               iterator.remove();
               int level = ((NumberTag)display.get("lvl")).asInt();
